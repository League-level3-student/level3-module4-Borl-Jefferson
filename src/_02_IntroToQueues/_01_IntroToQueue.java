...
while(stdb.size()>0){
int rf =r.nextInt(5)+1;
System.out.println("\n------------------------------------\nRemoving " + rf + " elements from Queue:");
for(int j =0; j<rf; j++) {
	qdb.add(stdb.pop());
}
for(int k = 0; k<rf; k++) {
	System.out.print(" "+ qdb.remove());
}
}
